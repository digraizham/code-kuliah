...
@time begin
    ...
    data_original = data
    data_hasil = balanced_resampling(data_original, data, 2, 0.01)

    new_filename = "balanced_resampled_data.csv"
    CSV.write(new_filename, data_hasil)

    dataSetosa = filter(row -> row[5] == "Iris-setosa", data_hasil)
    dataVersicolor = filter(row -> row[5] == "Iris-versicolor", data_hasil)
    dataVirginica = filter(row -> row[5] == "Iris-virginica", data_hasil)

    dataSetosa = dataSetosa[1:10000, :]
    dataVersicolor = dataVersicolor[1:10000, :]
    dataVirginica = dataVirginica[1:10000, :]

    scatter(dataSetosa[!, 1], dataSetosa[!, 2], label="Setosa", color="red", shape=:plus, marksize=7)
    scatter!(dataVersicolor[!, 1], dataVersicolor[!, 2], label="Versicolor", color="green", shape=:star, marksize=7)
    scatter!(dataVirginica[!, 1], dataVirginica[!, 2], label="Virginica", color="blue", shape=:diamond, marksize=7)
